...
export function MeshEditorModal({ onClose }: { onClose: () => void }) {
  ...
  const applyPalette = (palette: readonly string[]) => setPoints(background.meshPoints.map((point, index) => ({ ...point, color: palette[index % palette.length] })));
  const addPoint = () => {
    const point: MeshPoint = { id: crypto.randomUUID(), x: 50, y: 50, color: '#86c9ff', size: 42, opacity: 90 };
  ...
  return <div role="dialog" aria-modal="true" aria-label="Mesh background editor" className="fixed inset-0 z-[200] grid place-items-center bg-ink/25 p-5 backdrop-blur-sm" onPointerDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <div className="flex h-[min(720px,calc(100vh-32px))] w-[min(1040px,calc(100vw-32px))] flex-col overflow-hidden rounded-3xl border border-white/70 bg-surface shadow-2xl shadow-ink/25">
      <header className="flex items-center justify-between border-b border-border px-5 py-3.5"><div><h2 className="text-sm font-semibold">Mesh studio</h2><p className="text-[10px] text-muted">Build a soft multi-point background. Drag the lights directly.</p></div><div className="flex items-center gap-2"><button type="button" onClick={() => setPoints(createDefaultMeshPoints())} className="flex items-center gap-1.5 rounded-xl border border-border px-3 py-2 text-[10px] font-semibold hover:bg-cream-50"><RotateCcw className="size-3" /> Reset</button><button type="button" onClick={onClose} aria-label="Close mesh editor" className="rounded-xl p-2 hover:bg-cream-100"><X className="size-4" /></button></div></header>
      <div className="grid min-h-0 flex-1 grid-cols-[minmax(0,1fr)_270px]">
        ...
        <aside className="min-h-0 overflow-y-auto border-l border-border p-4">
          ...
          <div className="grid grid-cols-5 gap-1.5">{LAYOUTS.map((layout) => <button key={layout.label} type="button" title={layout.label} onClick={() => setPoints(background.meshPoints.map((point, index) => ({ ...point, x: layout.positions[index % layout.positions.length][0], y: layout.positions[index % layout.positions.length][1] })))} className="relative aspect-square rounded-lg border border-border bg-primary-50">{layout.positions.map(([x, y], index) => <span key={index} className="absolute size-1.5 -translate-x-1/2 -translate-y-1/2 rounded-full bg-primary-500" style={{ left: `${x}%`, top: `${y}%` }} />)}</button>)}</div>
          ...
          <div className="mb-3 flex items-center justify-between"><p className="text-xs font-semibold">Color points</p><span className="rounded-full bg-primary-50 px-2 py-1 text-[9px] text-primary-700">{background.meshPoints.length} points</span></div>
          ...
          {selected && <div className="space-y-3 rounded-2xl border border-border bg-cream-50 p-3"><label className="flex items-center justify-between text-[10px] text-muted">Point color <input type="color" value={selected.color} onChange={(event) => patchPoint(selected.id, { color: event.target.value })} className="size-8 cursor-pointer rounded-lg border-0 bg-transparent" /></label><EditorRange label="Glow size" value={selected.size} min={12} max={80} suffix="%" onChange={(size) => patchPoint(selected.id, { size })} /><EditorRange label="Opacity" value={selected.opacity} min={0} max={100} suffix="%" onChange={(opacity) => patchPoint(selected.id, { opacity })} /><button type="button" disabled={background.meshPoints.length <= 2} onClick={() => { setPoints(background.meshPoints.filter((point) => point.id !== selected.id)); setSelectedId(background.meshPoints.find((point) => point.id !== selected.id)?.id); }} className="flex w-full items-center justify-center gap-1.5 rounded-xl border border-accent-200 py-2 text-[10px] font-semibold text-danger disabled:opacity-30"><Trash2 className="size-3" /> Remove point</button></div>}
          <div className="my-4 h-px bg-border" />
          <p className="mb-2 text-[9px] font-semibold uppercase tracking-wider text-muted">Texture</p><div className="mb-3 grid grid-cols-4 gap-1">{NOISES.map((noise) => <button key={noise.value} type="button" onClick={() => update({ noiseType: noise.value, noise: Math.max(10, background.noise) })} className={`rounded-lg border p-1 ${background.noiseType === noise.value ? 'border-primary-400' : 'border-border'}`}><span className="block h-6 rounded bg-primary-200" style={getNoiseStyle(noise.value)} /><span className="mt-1 block text-[7px] text-muted">{noise.label}</span></button>)}</div>
