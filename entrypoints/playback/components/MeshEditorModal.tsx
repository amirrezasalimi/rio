    ...
    <div className="flex h-[min(720px,calc(100vh-32px))] w-[min(1040px,calc(100vw-32px))] flex-col overflow-hidden rounded-3xl border border-white/70 bg-surface shadow-2xl shadow-ink/25">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-[minmax(0,1fr)_270px]">
        ...
        <aside className="min-h-0 overflow-y-auto border-l border-border p-4">
          ...
          {selected && <div className="space-y-3 rounded-2xl border border-border bg-cream-50 p-3"><label className="flex items-center justify-between text-[10px] text-muted">Point color <input type="color" value={selected.color} onChange={(event) => patchPoint(selected.id, { color: event.target.value })} onBlur={() => useHistoryStore.getState().record('Change mesh point color')} className="size-8 cursor-pointer rounded-lg border-0 bg-transparent" /></label><EditorRange label="Glow size" value={selected.size} min={12} max={80} suffix="%" onChange={(size) => patchPoint(selected.id, { size })} /><EditorRange label="Opacity" value={selected.opacity} min={0} max={100} suffix="%" onChange={(opacity) => patchPoint(selected.id, { opacity })} /><button type="button" disabled={background.meshPoints.length <= 2} onClick={() => { useHistoryStore.getState().record('Remove mesh point'); setPoints(background.meshPoints.filter((point) => point.id !== selected.id)); setSelectedId(background.meshPoints.find((point) => point.id !== selected.id)?.id); }} className="flex w-full items-center justify-center gap-1.5 rounded-xl border border-accent-200 py-2 text-[10px] font-semibold text-danger disabled:opacity-30"><Trash2 className="size-3" /> Remove point</button></div>}
          <div className="my-4 h-px bg-border" />
