...
export function ZoomTimelineLane({ timelineDurationMs, onDragStateChange }: { timelineDurationMs: number; onDragStateChange: (duration: number | undefined) => void }) {
  ...
  return <div ref={trackRef} className="relative h-14 shrink-0 border-t border-border/70 bg-[var(--color-timeline-zoom)]">
    {clips.map((clip, index) => {
      ...
      return <div key={clip.id} data-clip className="absolute bottom-1.5 top-1.5 min-w-16 touch-none" style={{ left: `${clip.timelineStartMs / timelineDurationMs * 100}%`, width: `${clip.durationMs / timelineDurationMs * 100}%` }}>
        <div role="button" tabIndex={0} aria-label="Zoom clip. Click to add a zoom point." onPointerDown={(event) => { if (event.currentTarget === event.target) addPoint(event, clip.id); }} className={`absolute inset-0 cursor-crosshair rounded-lg border bg-control/70 ${selected ? 'border-selection-border ring-1 ring-selection-border/30' : 'border-border hover:border-muted/60'}`}>
          <span className="pointer-events-none absolute left-6 top-1 flex items-center gap-1 text-[9px] font-semibold text-muted"><Search className="size-3" /> Zoom · click to add</span>
          {clip.points.map((point) => <button key={point.id} type="button" aria-label={`Zoom point ${point.zoom.toFixed(1)} times`} onPointerDown={(event) => dragPoint(event, clip.id, point.id)} className={`absolute bottom-1/2 z-20 -translate-x-1/2 translate-y-1/2 rounded-full border-2 bg-transparent transition ${point.zoom > 1.01 ? 'size-6 border-muted' : 'size-3 border-muted/60'} ${clip.selectedPointId === point.id ? 'border-selection-border ring-1 ring-selection-border ring-offset-1 ring-offset-control' : ''}`} style={{ left: `${point.timeMs / clip.durationMs * 100}%` }} />)}
        </div>
        <button type="button" aria-label="Trim start of zoom clip" onPointerDown={(event) => startClipDrag(event, index, 'start')} className="absolute inset-y-0 left-0 z-30 flex w-4 cursor-ew-resize items-center justify-center rounded-l-lg bg-[var(--color-timeline-handle)] text-muted"><GripVertical className="size-3" /></button>
        <button type="button" aria-label="Trim end of zoom clip" onPointerDown={(event) => startClipDrag(event, index, 'end')} className="absolute inset-y-0 right-0 z-30 flex w-4 cursor-ew-resize items-center justify-center rounded-r-lg bg-[var(--color-timeline-handle)] text-muted"><GripVertical className="size-3" /></button>
