...
function UnsplashExplorer({ anchor, onClose, onSelect }: { anchor: DOMRect; onClose: () => void; onSelect: (photo: UnsplashPhoto) => void }) {
  ...
  return <div ref={panelRef} role="dialog" aria-label="Unsplash explorer" className="fixed z-[100] flex h-[min(610px,calc(100vh-24px))] w-[min(620px,calc(100vw-340px))] flex-col overflow-hidden rounded-2xl border border-border bg-surface shadow-2xl shadow-ink/20" style={{ left: Math.min(anchor.right + 10, window.innerWidth - 630), top }} onPointerDown={(event) => event.stopPropagation()}>
    <header className="flex items-center justify-between border-b border-border px-4 py-3"><div><h3 className="text-sm font-semibold">Unsplash explorer</h3><p className="text-[10px] text-muted">Search without leaving your canvas.</p></div><button type="button" aria-label="Close Unsplash explorer" onClick={onClose} className="rounded-lg p-2 hover:bg-control-hover"><X className="size-4" /></button></header>
    <div className="border-b border-border p-3"><div className="flex gap-2"><label className="flex min-w-0 flex-1 items-center gap-2 rounded-xl border border-border bg-control px-3 focus-within:border-primary-300"><Search className="size-3.5 text-muted" /><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void search(); }} placeholder="Search photos" className="min-w-0 flex-1 bg-transparent py-2 text-xs outline-none" /></label><button type="button" onClick={() => void search()} className="rounded-xl bg-primary-500 px-4 text-xs font-semibold text-on-primary">Search</button></div><div className="mt-2 flex flex-wrap gap-1.5">{CATEGORIES.map((category) => <button type="button" key={category} onClick={() => { setQuery(category); void search(category); }} className="rounded-full border border-border px-2 py-1 text-[9px] text-muted hover:border-primary-300">{category}</button>)}</div>{error && <p role="alert" className="mt-2 text-[10px] text-danger">{error}</p>}</div>
    <div className="min-h-52 flex-1 overflow-y-auto p-3">{loading ? <div className="grid h-full place-items-center"><LoaderCircle className="size-5 animate-spin text-primary-500" /></div> : <div className="grid grid-cols-3 gap-2">{photos.map((photo) => <button type="button" key={photo.id} onClick={() => onSelect(photo)} className="group relative aspect-[4/3] overflow-hidden rounded-xl bg-cream-100"><img src={photo.urls.small} alt={photo.alt_description ?? ''} className="size-full object-cover transition duration-300 group-hover:scale-105" /><span className="absolute inset-x-0 bottom-0 truncate bg-ink/65 px-2 py-1 text-left text-[9px] text-white">{photo.user.name}</span></button>)}</div>}</div>
...
export function BackgroundPanel() {
  ...
  return (
    ...
    <div className="border-t border-border pt-3"><p className="mb-2 text-[9px] font-semibold uppercase tracking-wider text-muted">Texture</p><div className="grid grid-cols-4 gap-1.5">{NOISE_TYPES.map((noise) => <button key={noise.value} type="button" aria-pressed={background.noiseType === noise.value && background.noise > 0} onClick={() => { useHistoryStore.getState().record('Change noise style'); update({ noiseType: noise.value, noise: Math.max(background.noise, 12) }); }} className={`rounded-xl border p-1.5 ${background.noiseType === noise.value && background.noise > 0 ? 'border-selection-border bg-selection' : 'border-border'}`}><span className="block h-7 rounded-lg bg-primary-200" style={getNoiseStyle(noise.value)} /><span className="mt-1 block text-[9px] font-semibold text-muted">{noise.label}</span></button>)}</div><div className="mt-3 grid grid-cols-2 gap-2"><EditorRange label="Noise opacity" value={background.noise} min={0} max={40} suffix="%" onChange={(noise) => update({ noise })} /><EditorRange label="Blur" value={background.blur} min={0} max={40} suffix="px" onChange={(blur) => update({ blur })} /></div></div>
    {unsplashAnchor && <UnsplashExplorer anchor={unsplashAnchor} onClose={() => setUnsplashAnchor(undefined)} onSelect={(photo) => { useHistoryStore.getState().record('Select background photo'); selectUnsplash(photo); }} />}
