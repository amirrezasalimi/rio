...
export function BackgroundPanel() {
  ...
  return (
    ...
    <div className="border-t border-border pt-3"><p className="mb-2 text-[9px] font-semibold uppercase tracking-wider text-muted">Texture</p><div className="grid grid-cols-4 gap-1.5">{NOISE_TYPES.map((noise) => <button key={noise.value} type="button" onClick={() => { useHistoryStore.getState().record('Change noise style'); update({ noiseType: noise.value, noise: Math.max(background.noise, 12) }); }} className={`rounded-xl border p-1.5 ${background.noiseType === noise.value && background.noise > 0 ? 'border-primary-400 bg-primary-50' : 'border-border'}`}><span className="block h-7 rounded-lg bg-primary-200" style={getNoiseStyle(noise.value)} /><span className="mt-1 block text-[8px] font-semibold text-muted">{noise.label}</span></button>)}</div><div className="mt-3 grid grid-cols-2 gap-2"><EditorRange label="Noise opacity" value={background.noise} min={0} max={40} suffix="%" onChange={(noise) => update({ noise })} /><EditorRange label="Blur" value={background.blur} min={0} max={40} suffix="px" onChange={(blur) => update({ blur })} /></div></div>
