...
export function FileMenu({ currentId, busy, onImport, onExport, onDelete, onError }: FileMenuProps) {
  ...
  return (
    <div ref={rootRef} className="relative">
      <button type="button" aria-haspopup="menu" aria-expanded={open} onClick={() => setOpen((value) => !value)} className="rounded-lg px-2.5 py-1.5 text-[11px] font-semibold hover:bg-control-hover">File</button>
      ...
      {open && (
        <div role="menu" className="absolute left-0 top-full z-[100] mt-2 w-64 rounded-xl border border-border bg-surface p-1.5 shadow-xl shadow-ink/15">
          <button type="button" role="menuitem" onClick={() => void openNew()} className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-[11px] font-semibold hover:bg-control-hover"><FilePlus2 className="size-4 text-primary-600" /> New</button>
          <div className="relative" onPointerEnter={() => setRecentsOpen(true)} onPointerLeave={() => setRecentsOpen(false)}>
            <button type="button" role="menuitem" aria-haspopup="menu" aria-expanded={recentsOpen} onClick={() => setRecentsOpen((value) => !value)} className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-[11px] font-semibold hover:bg-control-hover"><FolderClock className="size-4 text-primary-600" /> <span className="flex-1">Recents</span><ChevronRight className="size-3.5" /></button>
            {recentsOpen && <div role="menu" className="absolute left-[calc(100%-4px)] top-0 w-72 rounded-xl border border-border bg-surface p-1.5 shadow-xl shadow-ink/15">
              {recents.length === 0 ? <p className="px-3 py-4 text-center text-[10px] text-muted">No recent projects</p> : recents.slice(0, 12).map((item) => <button type="button" role="menuitem" key={item.id} onClick={() => openRecent(item.id)} className={`flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left hover:bg-control-hover ${item.id === currentId ? 'bg-selection' : ''}`}><span className="min-w-0 flex-1"><span className="block truncate text-[10px] font-semibold">Recording {new Date(item.createdAt).toLocaleString()}</span><span className="block text-[9px] text-muted">{formatDuration(item.durationMs)}</span></span>{item.id === currentId && <span className="text-[9px] font-semibold text-primary-700">Open</span>}</button>)}</div>}
          </div>
          <button type="button" role="menuitem" disabled={busy} onClick={() => inputRef.current?.click()} className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-[11px] font-semibold hover:bg-control-hover disabled:opacity-40"><FolderInput className="size-4 text-primary-600" /> Import…</button>
          <button type="button" role="menuitem" disabled={!currentId || busy} onClick={() => { setOpen(false); void onExport(); }} className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-[11px] font-semibold hover:bg-control-hover disabled:opacity-40"><Archive className="size-4 text-primary-600" /> Export current as ZIP</button>
          <div className="my-1 h-px bg-border" />
          <button type="button" role="menuitem" disabled={!currentId || busy} onClick={() => { if (window.confirm('Delete this project, its original recording, and all imported media? This cannot be undone.')) { setOpen(false); void onDelete(); } }} className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-[11px] font-semibold text-danger hover:bg-danger-soft disabled:opacity-40"><Trash2 className="size-4" /> Delete current</button>
        </div>
      )}
    </div>
  );
