...
export function ZoomSettingsPanel() {
  ...
  return <>
    <InspectorSection icon={Focus} title="Target" summary={targets.find((target) => (target.value.kind === 'canvas' ? 'canvas' : `${target.value.kind}:${target.value.id}`) === targetValue)?.label} searchTerms="scene canvas recording media clip source" defaultOpen>
    ...
    {point && <InspectorSection icon={CircleDot} title="Selected point" summary={`${point.zoom}× · ${(point.timeMs / 1_000).toFixed(2)}s`} searchTerms="zoom level time position delete wheel" defaultOpen>
    ...
    <InspectorSection icon={CircleDot} title="Transition" summary={ANIMATIONS.find((animation) => animation.value === clip.animation)?.label} searchTerms="animation smooth ease spring snap duration timing">
