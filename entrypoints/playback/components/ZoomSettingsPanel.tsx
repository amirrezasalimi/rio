...
export function ZoomSettingsPanel() {
  ...
  return <>
    ...
    {point && <section className="border-b border-border px-4 py-4">
      <div className="mb-1 flex items-center justify-between"><div><p className="text-xs font-semibold">Selected zoom point</p><p className="text-[8px] text-muted">Drag on canvas · wheel changes level</p></div><button type="button" aria-label="Delete selected zoom point" onClick={() => { useHistoryStore.getState().record('Delete zoom point'); store.updateZoomClip(clip.id, { points: clip.points.filter((item) => item.id !== point.id), selectedPointId: undefined }); }} className="rounded-lg p-2 text-danger hover:bg-accent-50"><Trash2 className="size-3.5" /></button></div>
