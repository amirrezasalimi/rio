...
export function ZoomSettingsPanel() {
  ...
  return <>
    ...
    {point && <InspectorSection icon={CircleDot} title="Selected point" summary={`${point.zoom}× · ${(point.timeMs / 1_000).toFixed(2)}s`} defaultOpen>
      <div className="mb-1 flex items-center justify-between"><p className="text-[9px] text-muted">Drag on canvas · wheel changes level</p><button type="button" aria-label="Delete selected zoom point" onClick={() => { useHistoryStore.getState().record('Delete zoom point'); store.updateZoomClip(clip.id, { points: clip.points.filter((item) => item.id !== point.id), selectedPointId: undefined }); }} className="rounded-lg p-2 text-danger hover:bg-accent-50"><Trash2 className="size-3.5" /></button></div>
    ...
    <InspectorSection icon={CircleDot} title="Transition" summary={ANIMATIONS.find((animation) => animation.value === clip.animation)?.label}>
      <div className="space-y-1">{ANIMATIONS.map((animation) => <button key={animation.value} type="button" onClick={() => { useHistoryStore.getState().record('Change zoom animation'); store.updateZoomClip(clip.id, { animation: animation.value }); }} className={`flex w-full items-center justify-between rounded-xl border px-3 py-2 text-left transition ${clip.animation === animation.value ? 'border-primary-400 bg-primary-50' : 'border-border bg-cream-50 hover:border-primary-200'}`}><span className="text-[9px] font-semibold">{animation.label}</span><span className="text-[8px] text-muted">{animation.description}</span></button>)}</div>
