...
export function TimelineRuler({ currentTimeMs, timelineDurationMs, preciseTicks }: TimelineRulerProps) {
  ...
  return (
    <div className="sticky top-0 z-40 h-6 shrink-0 border-b border-border/70 bg-cream-100 font-mono text-[9px] text-muted">
      ...
      >
        <span className="rio-timeline-playhead pointer-events-none absolute bottom-0 left-1/2 size-3.5 -translate-x-1/2 translate-y-1/2 rotate-45 rounded-[4px] bg-[var(--color-timeline-playhead)] shadow-sm ring-1 ring-border transition-transform group-active:scale-110" />
      </button>
    </div>
  );
}
