...
export function ClipContextMenu({ label, onOpen, onDuplicate, onDelete }: { label: string; onOpen: () => void; onDuplicate: () => void; onDelete: () => void }) {
  ...
  useEffect(() => {
    ...
    const open = (event: MouseEvent) => {
      ...
      onOpen();
      setPosition({ x: Math.min(event.clientX, window.innerWidth - 152), y: Math.min(event.clientY, window.innerHeight - 96) });
    };
