...
export function TextTimelineLane({ timelineDurationMs, onDragStateChange }: { timelineDurationMs: number; onDragStateChange: (duration: number | undefined) => void }) {
  ...
  const startDrag = (event: React.PointerEvent, index: number, interaction: 'clip' | 'start' | 'end') => {
    ...
    const itemSelection = { kind: 'text' as const, id: initial.id };
    ...
    const move = (moveEvent: PointerEvent) => {
      ...
      setClips((current) => current.map((clip) => {
        ...
        if (interaction === 'start') {
          const startDelta = Math.max(-initial.timelineStartMs, Math.min(initial.durationMs - MIN_CLIP_MS, delta));
          return { ...clip, timelineStartMs: initial.timelineStartMs + startDelta, durationMs: initial.durationMs - startDelta };
        }
        return { ...clip, durationMs: Math.max(MIN_CLIP_MS, initial.durationMs + delta) };
      }));
    };
    const stop = () => {
