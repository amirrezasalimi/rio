  ...
  return (
    <section ref={workspaceRef} onPointerDown={panWorkspace} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = 'copy'; }} onDrop={handleDrop} className="relative min-h-0 flex-1 cursor-grab overflow-hidden bg-cream-100/45 active:cursor-grabbing [background-image:radial-gradient(var(--color-border)_1px,transparent_1px)] [background-size:18px_18px]">
      <div className="pointer-events-none absolute left-3 top-3 z-30 flex items-center gap-1 rounded-xl border border-border bg-surface/92 p-1 shadow-sm backdrop-blur">
        <button type="button" aria-label="Zoom out" onClick={() => setZoom((value) => Math.max(10, value - 10))} className="pointer-events-auto rounded-lg p-1.5 hover:bg-control-hover"><Minus className="size-3.5" /></button>
        <span className="w-12 text-center font-mono text-[9px] text-muted">{Math.round(zoom)}%</span>
        <button type="button" aria-label="Zoom in" onClick={() => setZoom((value) => Math.min(200, value + 10))} className="pointer-events-auto rounded-lg p-1.5 hover:bg-control-hover"><Plus className="size-3.5" /></button>
        <button type="button" aria-label="Fit canvas" onClick={fitCanvas} className="pointer-events-auto rounded-lg p-1.5 hover:bg-control-hover"><Maximize className="size-3.5" /></button>
        <span className="mx-0.5 h-4 w-px bg-border" />
        <button type="button" aria-label={showControls ? 'Hide video controls' : 'Show video controls'} onClick={() => setShowControls((value) => !value)} className="pointer-events-auto rounded-lg p-1.5 hover:bg-control-hover">{showControls ? <Eye className="size-3.5" /> : <EyeOff className="size-3.5" />}</button>
      </div>
      <button type="button" disabled={!canMoveSelection} onClick={() => onMovingMediaChange(!movingMedia)} className={`absolute right-3 top-3 z-30 flex items-center gap-1.5 rounded-xl border px-3 py-2 text-[10px] font-semibold shadow-sm disabled:cursor-not-allowed disabled:opacity-40 ${movingMedia ? 'border-selection-border bg-primary-500 text-on-primary' : 'border-border bg-surface text-ink hover:border-primary-300'}`}><MousePointer2 className="size-3.5" />{movingMedia ? 'Done moving' : 'Move selected'}</button>
