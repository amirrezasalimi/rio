...
export function TextSettingsPanel() {
  ...
  return (
    <section className="border-b border-border px-4 py-4">
      ...
      <label className="block text-[10px] text-muted">Content<textarea aria-label="Text content" rows={3} value={clip.text} onChange={(event) => updateTextClip(clip.id, { text: event.currentTarget.value })} onBlur={() => useHistoryStore.getState().record('Change text content')} className="mt-1 w-full resize-none rounded-xl border border-border bg-cream-50 px-3 py-2 text-xs text-ink outline-none transition focus:border-primary-400 focus:ring-2 focus:ring-primary-100" /></label>
      ...
      <div className="mt-3 flex items-end gap-2">
        <label className="min-w-0 flex-1 text-[10px] text-muted">Font family<select aria-label="Font family" value={clip.fontFamily} onChange={(event) => { useHistoryStore.getState().record('Change font family'); updateTextClip(clip.id, { fontFamily: event.currentTarget.value }); }} className="mt-1 block w-full rounded-xl border border-border bg-cream-50 px-2.5 py-2 text-[11px] text-ink outline-none focus:border-primary-400">{fonts.map((font) => <option key={font} value={font} style={{ fontFamily: font }}>{font}</option>)}</select></label>
      ...
      <div className="mt-4">
        ...
        <div className="grid grid-cols-2 gap-1.5">{(['solid', 'gradient'] as const).map((type) => <button key={type} type="button" onClick={() => { useHistoryStore.getState().record('Change fill type'); updateTextClip(clip.id, { fill: type === 'solid' ? { type, color: clip.fill.type === 'solid' ? clip.fill.color : clip.fill.colorA } : { type, colorA: clip.fill.type === 'gradient' ? clip.fill.colorA : clip.fill.color, colorB: '#328fdf', angle: 135 } }); }} className={`rounded-xl border px-2 py-2 text-[9px] font-semibold capitalize ${clip.fill.type === type ? 'border-primary-400 bg-primary-50 text-primary-800' : 'border-border bg-cream-50 text-muted'}`}>{type}</button>)}</div>
