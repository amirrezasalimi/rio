...
export function TextSettingsPanel() {
  ...
  return <>
    ...
    <InspectorSection icon={CaseSensitive} title="Typography" summary={`${clip.fontFamily} · ${clip.fontSize}px · ${clip.fontWeight}`} defaultOpen>
      ...
      <div className="mt-3 flex items-end gap-2"><label className="min-w-0 flex-1 text-[10px] text-muted">Font family<select aria-label="Font family" value={clip.fontFamily} onChange={(event) => { useHistoryStore.getState().record('Change font family'); updateTextClip(clip.id, { fontFamily: event.currentTarget.value }); }} className="mt-1 block w-full rounded-xl border border-border bg-cream-50 px-2.5 py-2 text-[11px] text-ink outline-none focus:border-primary-400">{fonts.map((font) => <option key={font} value={font} style={{ fontFamily: font }}>{font}</option>)}</select></label><button type="button" onClick={() => void loadInstalledFonts()} aria-label="Load installed fonts" title="Load installed fonts" className="rounded-xl border border-border bg-cream-50 p-2 text-primary-700 hover:border-primary-300"><RefreshCw className="size-3.5" /></button></div>
